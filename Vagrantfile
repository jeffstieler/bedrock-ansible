...
if File.exists?(config_file)
  ...
  wordpress_sites = config['wordpress_sites']
  node_apps = config['node_apps']
else
...
Vagrant.configure('2') do |config|
  ...
  all_sites = wordpress_sites.concat(node_apps)

  main_site, *other_sites = all_sites

  config.vm.hostname = main_site['site_hosts'].first

  if Vagrant.has_plugin? 'vagrant-hostsupdater'
    host_aliases = other_sites.select { |site| site.has_key?('site_hosts') }
    host_aliases = host_aliases.flat_map { |site| site['site_hosts'] }
